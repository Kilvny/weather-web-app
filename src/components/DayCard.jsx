...
const DayCard = ({ data, imgSrc }) => {
    const [weekdays, setWeekdays] = useState(['Mon','Tue','Wen','Thu','Fri','Sat','Sun'])
    useEffect(() => {
  ...
  return (
    <div className="overflow-x-auto flex hover:overflow-x-auto">
        {data? data.forecast?.forecastday?.map((day,i)=>(
                <div key={i} className="inline-block min-w-fit rounded-md border-2 border-sky-500 border-solid shadow-lg shadow-cyan-600/50 m-4 max-w-fit p-1 bg-gradient-to-r from-sky-900 to-rose-900">
                <div className="my-1 items-center justify-center flex flex-col m-auto text-center align-middle">
                  <p className="font-medium max-w-full text-center text-xs my-1 max-md:text-xs">
                    <p className="my-1 mb-2 text-sm italic capitalize text-gray-100">
                      {!day ? "" : weekdays[i] + " " + day.date.slice(5)}
                      {/* {console.log(day)} */}
                    </p>
                    ...
                    <span className="mx-2 text-sm">{!day ? "" : "Max: "+ day.day.maxtemp_c}&deg;</span>
                  </p>
